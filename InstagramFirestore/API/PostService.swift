...
struct PostService {
  ...
  static func fetchFeedPosts(completion: @escaping([Post]) -> Void) {
    ...
    if COLLECTION_USERS.document(uid).collection("user-feed").collectionID.isEqual(nil) { return }
    
    //get posts of user based on uid on "user-feed" collection.
    ...
    query.getDocuments { (snapshot, _) in
      ...
      docIDs.forEach { id in
        if didFollow {
          ...
          // Delete all doc IDs of user's post to user-feed collection.
          COLLECTION_USERS
            .document(uid)
            .collection("user-feed")
            .document(id)
  ...
  private static func updateUserFeedAfterPost(postID: String) {
    ...
    // Get all the followers based on uid.
    COLLECTION_FOLLOWERS
      ...
      // Update user-feed collection of users who followed.
      documents.forEach { document in
        COLLECTION_USERS
